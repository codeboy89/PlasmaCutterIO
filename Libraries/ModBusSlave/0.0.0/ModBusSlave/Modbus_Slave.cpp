...
//################## ModBusSlave ###################
// Takes:   Slave Address, Pointer to this->coilRegisters and Number of Available this->coilRegisters
// Returns: Nothing
...
// Takes:   In Data Buffer
// Returns: Nothing
// Effect:  Sets Reply Data and this->coilRegister values, sends Response for Write Single Reg.
...
// Takes:   In Data Buffer
// Returns: Nothing
// Effect:  Writes single bit in this->coilRegisters.  Sets Reply Data and sends Response
...
// Takes:   In Data Buffer
// Returns: Nothing
// Effect:  Writes this->coilRegister in this->coilRegisters.  Sets Reply Data and sends Response
...
// Takes:   In Data Buffer
// Returns: Nothing
// Effect:  Writes multiple bits in this->coilRegisters.  Sets Reply Data and sends Response
void ModBusSlave::Write_Coils(unsigned char *Data_In)  // Function code 1
{
	...
	for(int C = 0; C < Write_Bit_Count; C++) {
		if((Data_In[Read_Byte] & (1 << Read_Bit)) > 0)		// If set Bit is a 1, then, set corrisponding bit in this->coilRegister
		{
			Coils[Write_Address] |= (1 << Write_Bit);
		} else											// If set bit is a 0, clear the bit in the this->coilRegister
		{
			Coils[Write_Address] &= ~(1 << Write_Bit);
		}
		...
		if(Write_Bit >= 16)								// Write are to 16bit this->coilRegisters, so increment every 16th bit.	
		{
			Write_Bit = 0;
			Write_Address++;
		}
